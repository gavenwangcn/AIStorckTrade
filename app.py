...
def get_tracked_symbols():
    ...
    if not symbols:
        print('[WARN] No stocks configured. Please add stocks via /api/stocks.')
    return symbols
...
def add_stock_config():
    data = request.json or {}
    symbol = data.get('symbol', '').strip().upper()
    name = data.get('name', '').strip()
    exchange = data.get('exchange', '').strip().upper()
    api_symbol = data.get('api_symbol', '').strip().upper()

    if not all([symbol, name, exchange, api_symbol]):
        return jsonify({'error': 'symbol, name, exchange, api_symbol are required'}), 400

    try:
        stock_id = db.add_stock(symbol, name, exchange, api_symbol)
        return jsonify({'id': stock_id, 'message': 'Stock added successfully'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/stocks/<int:stock_id>', methods=['DELETE'])
def delete_stock_config(stock_id):
    try:
...
def update_settings():
    ...
    try:
        ...
        auto_trading_start = data.get('auto_trading_start', '09:30:00') or '09:30:00'
        auto_trading_end = data.get('auto_trading_end', '15:00:00') or '15:00:00'
