...
class Database:
    ...
    def init_db(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS settings (
                ...
                auto_trading_start TEXT DEFAULT '09:30:00',
                auto_trading_end TEXT DEFAULT '15:30:00',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        ''')
        ...
        if 'auto_trading_start' not in settings_columns:
            cursor.execute("ALTER TABLE settings ADD COLUMN auto_trading_start TEXT DEFAULT '09:30:00'")
        if 'auto_trading_end' not in settings_columns:
            cursor.execute("ALTER TABLE settings ADD COLUMN auto_trading_end TEXT DEFAULT '15:30:00'")

        # Stocks table (for A-share configuration)
        cursor.execute('''
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute('''
                INSERT INTO settings (trading_frequency_minutes, trading_fee_rate, show_system_prompt, auto_trading_start, auto_trading_end)
                VALUES (60, 0.001, 0, '09:30:00', '15:30:00')
            ''')
        ...
        if cursor.fetchone()[0] == 0:
            default_stocks = getattr(app_config, 'INITIAL_STOCKS', [])
            if default_stocks:
                cursor.executemany('''
                    INSERT INTO stocks (symbol, name, exchange, api_symbol)
                    VALUES (?, ?, ?, ?)
                ''', default_stocks)
    ...
    def get_stocks(self) -> List[Dict]:
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM stocks ORDER BY created_at DESC, symbol ASC')
        rows = cursor.fetchall()
        conn.close()
        return [dict(row) for row in rows]

    def add_stock(self, symbol: str, name: str, exchange: str, api_symbol: str) -> int:
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO stocks (symbol, name, exchange, api_symbol)
            VALUES (?, ?, ?, ?)
        ''', (symbol.upper(), name, exchange.upper(), api_symbol.upper()))
    ...
    def get_stock_configs(self) -> List[Dict]:
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT symbol, name, exchange, api_symbol FROM stocks ORDER BY symbol ASC')
        rows = cursor.fetchall()
        conn.close()
        return [dict(row) for row in rows]

    def get_stock_symbols(self) -> List[str]:
        return [stock['symbol'] for stock in self.get_stock_configs()]
    ...
    def get_settings(self) -> Dict:
        ...
        cursor.execute('''
            SELECT trading_frequency_minutes, trading_fee_rate,
                   ...
                   COALESCE(auto_trading_start, '09:30:00') as auto_trading_start,
                   COALESCE(auto_trading_end, '15:00:00') as auto_trading_end
            FROM settings
            ORDER BY id DESC
            LIMIT 1
        ''')
        ...
        if row:
            return {
                ...
                'auto_trading_start': row['auto_trading_start'],
                'auto_trading_end': row['auto_trading_end']
            }
        else:
    ...
    def update_settings(self, trading_frequency_minutes: int, trading_fee_rate: float,
                        show_system_prompt: int, auto_trading_start: str, auto_trading_end: str) -> bool:
        ...
        try:
            ...
            ''', (trading_frequency_minutes, trading_fee_rate, show_system_prompt, auto_trading_start, auto_trading_end))
