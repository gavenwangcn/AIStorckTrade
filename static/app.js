class TradingApp {
    ...
    initEventListeners() {
        ...
        // Stock Config Modal
        document.getElementById('stockConfigBtn').addEventListener('click', () => this.showStockModal());
        document.getElementById('closeStockModalBtn').addEventListener('click', () => this.hideStockModal());
    ...
    showStockModal() {
        this.loadStocks();
        document.getElementById('stockConfigModal').classList.add('show');
    }

    hideStockModal() {
        document.getElementById('stockConfigModal').classList.remove('show');
        this.clearStockForm();
    }
    ...
    clearStockForm() {
        ...
        document.getElementById('stockExchange').value = '';
        document.getElementById('stockApiSymbol').value = '';
    }
    ...
    renderStocks(stocks) {
        ...
        if (!stocks || stocks.length === 0) {
            container.innerHTML = '<div class="empty-state">暂无股票配置</div>';
            return;
        }
        ...
        container.innerHTML = stocks.map(stock => `
            <div class="provider-item">
                <div class="provider-info">
                    <div class="provider-name">${stock.symbol} - ${stock.name}</div>
                    <div class="provider-url">${stock.exchange} / ${stock.api_symbol}</div>
                </div>
                <div class="provider-actions">
                    <span class="provider-delete" onclick="app.deleteStock(${stock.id})" title="删除">
                        <i class="bi bi-trash"></i>
                    </span>
                </div>
            </div>
    ...
    async saveStock() {
        const data = {
            ...
            exchange: document.getElementById('stockExchange').value.trim(),
            api_symbol: document.getElementById('stockApiSymbol').value.trim()
        };

        if (!data.symbol || !data.name || !data.exchange || !data.api_symbol) {
            alert('请填写所有股票字段');
            return;
        }
